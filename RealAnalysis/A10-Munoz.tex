...
\begin{proof}
  \textbf{(i)} Since $f$ and $g$ are differentiable at some point $c \in A$, then $f'(c) = \lim_{h \to \infty} \frac{f(c+h) - f(c)}{h}$ and $g'(c) = \lim_{h \to \infty} \frac{g(c+h) - g(c)}{h}$ by definition. We want to show that $(f+g)'(c) = f'(c) + g'(c)$. Observe that
  \begin{align*}
  \end{align*}
\end{proof}
